...
def predict_2():
    """Home Page"""
    return render_template('website_home_copy_F.html')

@app.route('/twitter')
def twitter():
    return render_template('Twitter page/Login_v12/index2.html')
...
@app.route('/instagram')
def instagram():
    return render_template('Insta page/Login_v13/index2.html')

@app.route('/reddit')
def reddit():
    return render_template('Reddit page/Login_v14/index2.html')

@app.route('/redditoutput', methods =["GET", "POST"])
def redditoutput():
    if request.method == "POST":
        # getting input with name = fname in HTML form
        # getting input with name = lname in HTML form
        # return "Hash value is " +HashValue
        HashValue = request.form.get("HashValue")
        reddit = praw.Reddit(client_id='ZcaA_A1kp6hQpFv1kq77nA',
                            client_secret='jomDEd2dpo2eLJKscfRRE5_on97uPw',
                            user_agent='Sup')
        
        for submission in reddit.subreddit(HashValue).hot(limit=100):
            if submission.subreddit:
        # submission.url will give the url to the video
                print(submission.url)
                image_filename1 = wget.download(submission.url)
                print('Image Successfully Downloaded: ', image_filename1)
                label, score, fake_image_path = Fake_Face(image_filename1)
                print("label of twitter image:",label,"Predicted Fake Score of twitter image:",score)
                
    return render_template('redditoutput/redditoutput.html')
...
def news():
    ...
    return render_template('news/news.html', context=mylist)
